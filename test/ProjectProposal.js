...
describe("ProjectProposal Contract", function () {
  ...
  describe("Proposal Management", function () {
    ...
    it("Should revert when adding a proposal with zero amount", async function () {
      ...
      await expect(projectProposal.connect(addr1).addProposal(0)).to.be.revertedWithCustomError(projectProposal, "InvalidAmountRequested");
    });
    ...
    it("Should revert when adding a proposal with amount greater than round's maxFlareAmount", async function () {
      ...
      await expect(projectProposal.connect(addr1).addProposal(ethers.parseUnits("200", 18))).to.be.revertedWithCustomError(
        projectProposal,
    ...
    it("Should revert if trying to add a proposal outside submission window", async function () {
      ...
      await ethers.provider.send("evm_mine", []);
      await expect(projectProposal.connect(addr1).addProposal(ethers.parseUnits("10", 18))).to.be.revertedWithCustomError(
        projectProposal,
  ...
  describe("Round Management", function () {
    ...
    it("Should allow taking a snapshot", async function () {
      await projectProposal.addRound(ethers.parseUnits("10", 18), 3600, currentTime + 1800, { value: ethers.parseUnits("10", 18) });
    ...
    it("Should revert if snapshot is taken before expectedSnapshotDatetime", async function () {
      ...
      await expect(projectProposal.connect(owner).takeSnapshot()).to.be.revertedWithCustomError(projectProposal, "SubmissionWindowOpen");
    });
    ...
    it("Should extend the round runtime by the correct amount.", async function () {
      ...
      const latestRound = await projectProposal.getRoundById(1);
  ...
  describe("Voting", function () {
    ...
    it("Should give the user their voting power back after all votes from all proposals have been removed", async function () {
      ...
      expect(await projectProposal.getRemainingVotingPower(addr2.address)).to.equal(newPower);

      //Remove all votes.
      await projectProposal.connect(addr2).removeAllVotesFromAllProposals();

      await expect(await projectProposal.getRemainingVotingPower(addr2.address)).to.equal(votingPowerBefore);
    });
    ...
    it("Should get the correct FlothPass voting power", async function () {
      ...
      // Approve the FlothPass contract to spend Floth tokens from addr1
      await floth.connect(addr1).approve(await flothPass.getAddress(), ethers.parseUnits("4000", 18));
    ...
    it("Should allow voting partial FlothPass voting power to a proposal", async function () {
      ...
      const remainingVotingPower = await projectProposal.getRemainingVotingPower(addr1.address);
      expect(remainingVotingPower).to.equal(flothPassVotingPower - BigInt(200));
    });
    ...
    it("Should retrieve proposal ID's and the number of votes for each", async function () {
      ...
      //Add 2 proposals
      await projectProposal.connect(addr1).addProposal(ethers.parseUnits("10", 18));
      await projectProposal.connect(addr2).addProposal(ethers.parseUnits("10", 18));
  ...
  describe("Claiming Funds", function () {
    ...
    it("Should allow the winner to claim funds on the 29th day", async function () {
      ...
      // Addr1 claims funds.
      const tx = await projectProposal.connect(addr1).claimFunds(1);
    ...
    it("Should send funds to the grant wallet if abstain wins a round", async function () {
      ...
      const balanceBefore = await ethers.provider.getBalance(await floth.grantFundWallet());
    ...
    it("Should revert if non-winner tries to claim funds", async function () {
      ...
      //Addr2 claims funds.
      await expect(projectProposal.connect(addr2).claimFunds(1)).to.be.revertedWithCustomError(projectProposal, "InvalidClaimer");
    });
    ...
    it("Should allow admin to reclaim after 30 days", async function () {
      ...
      //Balance of "floth.grantFundWallet"
      const balanceBefore = await ethers.provider.getBalance(await floth.grantFundWallet());
    ...
    it("Should revert if user tries to claim after 30 days.", async function () {
      ...
      await expect(projectProposal.connect(addr1).claimFunds(1)).to.be.revertedWithCustomError(projectProposal, "FundsClaimingPeriodExpired");
    });
  });
  ...
  describe("Getters", function () {
    ...
    it("Should get unclaimed winning proposals by round id - multi-round test", async function () {
      ...
      //Addr2 claim round 2.
      await projectProposal.connect(addr2).claimFunds(2);
