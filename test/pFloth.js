...
describe("pFLOTH Contract", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should not allow invalid presale times", async function () {
      const pFLOTHFactory = await ethers.getContractFactory("pFloth");
      await expect(pFLOTHFactory.deploy(endTime, startTime)).to.be.revertedWithCustomError(pFLOTH, "InvalidPresaleTime");
    });
  });
  ...
  describe("Presale", function () {
    ...
    it("Should revert if presale has not started", async function () {
      ...
      await expect(newPFLOTH.connect(addr1).presale({ value: ethers.parseUnits("1", 18) })).to.be.revertedWithCustomError(
        pFLOTH,
        "PresaleNotStarted"
      );
    });
    ...
    it("Should revert if minting exceeds MAX_SUPPLY", async function () {
      ...
      await expect(pFLOTHMock.connect(addr1).presale({ value: amountFLR })).to.be.revertedWithCustomError(pFLOTHMock, "ExceedsSupply");
    });
    ...
    it("Should revert if minting exceeds WALLET_LIMIT", async function () {
      ...
      await expect(pFLOTHMock.connect(addr1).presale({ value: amountFLR })).to.be.revertedWithCustomError(pFLOTHMock, "ExceedsWalletLimit");
    });
    ...
    it("Should handle combined scenarios", async function () {
      ...
      await expect(pFLOTH.connect(addr1).transfer(addr2.address, amountpFLOTH)).to.be.revertedWithCustomError(pFLOTH, "UnauthorizedTransfer");
      ...
      await expect(pFLOTH.connect(owner).recoverTokens(await pFLOTH.getAddress(), amountpFLOTH)).to.be.revertedWithCustomError(
        pFLOTH,
  ...
  describe("Presale Time Management", function () {
    ...
    it("Should not allow setting end time in the past", async function () {
      ...
      await expect(pFLOTH.setPresaleEndTime(currentTime - 1n)).to.be.revertedWithCustomError(pFLOTH, "InvalidPresaleTime");
    });
  });
  ...
  describe("Token Transfers", function () {
    it("Should handle authorized receiver transfers", async function () {
      ...
      await expect(pFLOTH.connect(addr1).transfer(addr2.address, amountFLR)).to.be.revertedWithCustomError(pFLOTH, "UnauthorizedTransfer");

      // Authorize receiver and try again
      await pFLOTH.connect(owner).setAuthorizedReceiver(addr2.address, true);
      await expect(pFLOTH.connect(addr1).transfer(addr2.address, amountFLR)).to.not.be.reverted;
    });
  });
  ...
  describe("Token Recovery", function () {
    ...
    it("Should allow owner to recover tokens", async function () {
      ...
      await expect(pFLOTH.connect(owner).recoverTokens(ERC20Mock.getAddress(), amount)).to.not.be.reverted;
    ...
    it("Should not allow recovering pFLOTH tokens", async function () {
      await expect(pFLOTH.connect(owner).recoverTokens(await pFLOTH.getAddress(), 100)).to.be.revertedWithCustomError(pFLOTH, "InvalidRecoveryToken");
    });
  });
  ...
  describe("Withdrawal", function () {
    ...
    it("Should allow owner to withdraw all FLR", async function () {
      ...
      await pFLOTH.connect(owner).withdrawTo(0, owner.address, true);
    ...
    it("Should allow partial withdrawals to specified address", async function () {
      ...
      await pFLOTH.connect(owner).withdrawTo(withdrawAmount, addr2.address, false);
